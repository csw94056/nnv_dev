classdef Shape_To_ConstantOfShapeLayer1015 < nnet.layer.Layer & nnet.layer.Formattable
    ...
    methods
        function this = Shape_To_ConstantOfShapeLayer1015(name, onnxParams)
            this.Name = name;
            this.OutputNames = {'onnx__Cast_16'};
            this.ONNXParams = onnxParams;
        end
        
        function [onnx__Cast_16] = predict(this, input)
            if isdlarray(input)
                input = stripdims(input);
            end
            inputNumDims = 3;
            onnxParams = this.ONNXParams;
            [onnx__Cast_16, onnx__Cast_16NumDims] = Shape_To_ConstantOfShapeFcn(input, inputNumDims, onnxParams, 'Training', false, ...
                'InputDataPermutation', {[2 3 1], ['as-is']}, ...
                'OutputDataPermutation', {[3 2 1], ['as-is']});
            if any(cellfun(@(A)~isnumeric(A), {onnx__Cast_16}))
                fprintf('Runtime error in network. The custom layer ''%s'' output a non-numeric value.\n', 'Shape_To_ConstantOfShapeLayer1015');
                error(message('nnet_cnn_onnx:onnx:BadCustomLayerRuntimeOutput', 'Shape_To_ConstantOfShapeLayer1015'));
            end
            onnx__Cast_16 = dlarray(single(onnx__Cast_16), 'CB');
            if ~coder.target('MATLAB')
                onnx__Cast_16 = extractdata(onnx__Cast_16);
            end
        end
        
        function [onnx__Cast_16] = forward(this, input)
            if isdlarray(input)
                input = stripdims(input);
            end
            inputNumDims = 3;
            onnxParams = this.ONNXParams;
            [onnx__Cast_16, onnx__Cast_16NumDims] = Shape_To_ConstantOfShapeFcn(input, inputNumDims, onnxParams, 'Training', true, ...
                'InputDataPermutation', {[2 3 1], ['as-is']}, ...
                'OutputDataPermutation', {[3 2 1], ['as-is']});
            if any(cellfun(@(A)~isnumeric(A), {onnx__Cast_16}))
                fprintf('Runtime error in network. The custom layer ''%s'' output a non-numeric value.\n', 'Shape_To_ConstantOfShapeLayer1015');
...
function [onnx__Cast_16, onnx__Cast_16NumDims, state] = Shape_To_ConstantOfShapeFcn(input, inputNumDims, params, varargin)
%SHAPE_TO_CONSTANTOFSHAPEFCN Function implementing an imported ONNX network.
%
...
%
% [ONNX__CAST_16] = Shape_To_ConstantOfShapeFcn(INPUT, PARAMS)
%			- Evaluates the imported ONNX network SHAPE_TO_CONSTANTOFSHAPEFCN with input(s)
%			INPUT and the imported network parameters in PARAMS. Returns
%			network output(s) in ONNX__CAST_16.
%
% [ONNX__CAST_16, STATE] = Shape_To_ConstantOfShapeFcn(INPUT, PARAMS)
%			- Additionally returns state variables in STATE. When training,
%			use this form and set TRAINING to true.
%
% [__] = Shape_To_ConstantOfShapeFcn(INPUT, PARAMS, 'NAME1', VAL1, 'NAME2', VAL2, ...)
...
%				  INPUT:		[batch_size, 28, 28]				Type: FLOAT
...
%				  ONNX__CAST_16:		[Unknown, Unknown, Unknown]				Type: FLOAT
...
[onnx__Cast_16, onnx__Cast_16NumDims, state] = Shape_To_ConstantOfSGraph1010(input, NumDims.input, Vars, NumDims, Training, params.State);
% Postprocess the output data
[onnx__Cast_16] = postprocessOutput(onnx__Cast_16, outputDataPerms, anyDlarrayInputs, Training, varargin{:});
end

function [onnx__Cast_16, onnx__Cast_16NumDims1014, state] = Shape_To_ConstantOfSGraph1010(input, inputNumDims1013, Vars, NumDims, Training, state)
% Function implementing the graph 'Shape_To_ConstantOfSGraph1010'
...
% Shape:
[Vars.onnx__Gather_7, NumDims.onnx__Gather_7] = onnxShape(Vars.input, NumDims.input);

% Gather:
[Vars.onnx__Unsqueeze_9, NumDims.onnx__Unsqueeze_9] = onnxGather(Vars.onnx__Gather_7, Vars.onnx__Gather_8, 0, NumDims.onnx__Gather_7, NumDims.onnx__Gather_8);

% Unsqueeze:
[shape, NumDims.onnx__Concat_13] = prepareUnsqueezeArgs(Vars.onnx__Unsqueeze_9, Vars.UnsqueezeAxes1011, NumDims.onnx__Unsqueeze_9);
Vars.onnx__Concat_13 = reshape(Vars.onnx__Unsqueeze_9, shape);

% Concat:
[Vars.onnx__ConstantOfShap, NumDims.onnx__ConstantOfShap] = onnxConcat(0, {Vars.onnx__Concat_89, Vars.onnx__Concat_13, Vars.onnx__Concat_90}, [NumDims.onnx__Concat_89, NumDims.onnx__Concat_13, NumDims.onnx__Concat_90]);

% ConstantOfShape:
[Vars.onnx__Cast_16, NumDims.onnx__Cast_16] = onnxConstantOfShape(Vars.ConstantOfShapeValue1012, Vars.onnx__ConstantOfShap);

% Set graph output arguments from Vars and NumDims:
onnx__Cast_16 = Vars.onnx__Cast_16;
onnx__Cast_16NumDims1014 = NumDims.onnx__Cast_16;
% Set output state from Vars:
state = updateStruct(state, Vars);
end

function [inputDataPerms, outputDataPerms, Training] = parseInputs(input, numDataOutputs, params, varargin)
% Function to validate inputs to Shape_To_ConstantOfShapeFcn:
p = inputParser;
...
function [onnx__Cast_16] = postprocessOutput(onnx__Cast_16, outputDataPerms, anyDlarrayInputs, Training, varargin)
% Set output type:
if ~anyDlarrayInputs && ~Training
    if isdlarray(onnx__Cast_16)
        onnx__Cast_16 = extractdata(onnx__Cast_16);
    end
end
% Permute outputs if requested:
onnx__Cast_16 = permuteOutputVar(onnx__Cast_16, outputDataPerms{1}, 3);
end
