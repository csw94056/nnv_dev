classdef Shape_To_GemmLayer1027 < nnet.layer.Layer & nnet.layer.Formattable
    ...
    methods
        function this = Shape_To_GemmLayer1027(name, onnxParams)
            this.Name = name;
        ...
        function [output] = predict(this, onnx__Shape_79)
            if isdlarray(onnx__Shape_79)
                onnx__Shape_79 = stripdims(onnx__Shape_79);
            end
            onnx__Shape_79NumDims = 3;
            onnxParams = this.ONNXParams;
            onnxParams.Learnables.fc_bias = this.fc_bias;
            onnxParams.Learnables.fc_weight = this.fc_weight;
            [output, outputNumDims] = Shape_To_GemmFcn(onnx__Shape_79, onnx__Shape_79NumDims, onnxParams, 'Training', false, ...
                'InputDataPermutation', {[2 3 1], ['as-is']}, ...
                'OutputDataPermutation', {['as-is'], ['as-is']});
            if any(cellfun(@(A)~isnumeric(A), {output}))
                fprintf('Runtime error in network. The custom layer ''%s'' output a non-numeric value.\n', 'Shape_To_GemmLayer1027');
                error(message('nnet_cnn_onnx:onnx:BadCustomLayerRuntimeOutput', 'Shape_To_GemmLayer1027'));
            end
        ...
        function [output] = forward(this, onnx__Shape_79)
            if isdlarray(onnx__Shape_79)
                onnx__Shape_79 = stripdims(onnx__Shape_79);
            end
            onnx__Shape_79NumDims = 3;
            ...
            onnxParams.Learnables.fc_bias = this.fc_bias;
            onnxParams.Learnables.fc_weight = this.fc_weight;
            [output, outputNumDims] = Shape_To_GemmFcn(onnx__Shape_79, onnx__Shape_79NumDims, onnxParams, 'Training', true, ...
                'InputDataPermutation', {[2 3 1], ['as-is']}, ...
                'OutputDataPermutation', {['as-is'], ['as-is']});
            if any(cellfun(@(A)~isnumeric(A), {output}))
                fprintf('Runtime error in network. The custom layer ''%s'' output a non-numeric value.\n', 'Shape_To_GemmLayer1027');
...
function [output, outputNumDims, state] = Shape_To_GemmFcn(onnx__Shape_79, onnx__Shape_79NumDims, params, varargin)
%SHAPE_TO_GEMMFCN Function implementing an imported ONNX network.
%
...
%
% [OUTPUT] = Shape_To_GemmFcn(ONNX__SHAPE_79, PARAMS)
%			- Evaluates the imported ONNX network SHAPE_TO_GEMMFCN with input(s)
%			ONNX__SHAPE_79 and the imported network parameters in PARAMS. Returns
%			network output(s) in OUTPUT.
%
% [OUTPUT, STATE] = Shape_To_GemmFcn(ONNX__SHAPE_79, PARAMS)
%			- Additionally returns state variables in STATE. When training,
%			use this form and set TRAINING to true.
%
% [__] = Shape_To_GemmFcn(ONNX__SHAPE_79, PARAMS, 'NAME1', VAL1, 'NAME2', VAL2, ...)
...
%				  ONNX__SHAPE_79:		[Unknown, Unknown, Unknown]				Type: FLOAT
...
%				  OUTPUT:		[batch_size, Gemmoutput_dim_1]				Type: FLOAT
...
[onnx__Shape_79, Training, outputDataPerms, anyDlarrayInputs] = preprocessInput(onnx__Shape_79, params, varargin{:});
% Put all variables into a single struct to implement dynamic scoping:
[Vars, NumDims] = packageVariables(params, {'onnx__Shape_79'}, {onnx__Shape_79}, [onnx__Shape_79NumDims]);
% Call the top-level graph function:
[output, outputNumDims, state] = Shape_To_GemmGraph1021(onnx__Shape_79, NumDims.onnx__Shape_79, Vars, NumDims, Training, params.State);
% Postprocess the output data
[output] = postprocessOutput(output, outputDataPerms, anyDlarrayInputs, Training, varargin{:});
end

function [output, outputNumDims1026, state] = Shape_To_GemmGraph1021(onnx__Shape_79, onnx__Shape_79NumDims1025, Vars, NumDims, Training, state)
% Function implementing the graph 'Shape_To_GemmGraph1021'
% Update Vars and NumDims from the graph's formal input parameters. Note that state variables are already in Vars.
Vars.onnx__Shape_79 = onnx__Shape_79;
NumDims.onnx__Shape_79 = onnx__Shape_79NumDims1025;

% Execute the operators:
% Shape:
[Vars.onnx__Gather_80, NumDims.onnx__Gather_80] = onnxShape(Vars.onnx__Shape_79, NumDims.onnx__Shape_79);

% Gather:
[Vars.onnx__Unsqueeze_82, NumDims.onnx__Unsqueeze_82] = onnxGather(Vars.onnx__Gather_80, Vars.onnx__Gather_81, 0, NumDims.onnx__Gather_80, NumDims.onnx__Gather_81);

% Unsqueeze:
[shape, NumDims.onnx__Concat_84] = prepareUnsqueezeArgs(Vars.onnx__Unsqueeze_82, Vars.UnsqueezeAxes1022, NumDims.onnx__Unsqueeze_82);
Vars.onnx__Concat_84 = reshape(Vars.onnx__Unsqueeze_82, shape);

% Concat:
[Vars.onnx__Reshape_86, NumDims.onnx__Reshape_86] = onnxConcat(0, {Vars.onnx__Concat_84, Vars.onnx__Concat_111}, [NumDims.onnx__Concat_84, NumDims.onnx__Concat_111]);

% Reshape:
[shape, NumDims.onnx__Gemm_87] = prepareReshapeArgs(Vars.onnx__Shape_79, Vars.onnx__Reshape_86, NumDims.onnx__Shape_79, 0);
Vars.onnx__Gemm_87 = reshape(Vars.onnx__Shape_79, shape{:});

% Gemm:
[A, B, C, alpha, beta, NumDims.output] = prepareGemmArgs(Vars.onnx__Gemm_87, Vars.fc_weight, Vars.fc_bias, Vars.Gemmalpha1023, Vars.Gemmbeta1024, 0, 1, NumDims.fc_bias);
Vars.output = alpha*B*A + beta*C;

% Set graph output arguments from Vars and NumDims:
output = Vars.output;
outputNumDims1026 = NumDims.output;
% Set output state from Vars:
state = updateStruct(state, Vars);
end

function [inputDataPerms, outputDataPerms, Training] = parseInputs(onnx__Shape_79, numDataOutputs, params, varargin)
% Function to validate inputs to Shape_To_GemmFcn:
p = inputParser;
isValidArrayInput = @(x)isnumeric(x) || isstring(x);
isValidONNXParameters = @(x)isa(x, 'ONNXParameters');
addRequired(p, 'onnx__Shape_79', isValidArrayInput);
addRequired(p, 'params', isValidONNXParameters);
addParameter(p, 'InputDataPermutation', 'auto');
addParameter(p, 'OutputDataPermutation', 'auto');
addParameter(p, 'Training', false);
parse(p, onnx__Shape_79, params, varargin{:});
...
function [onnx__Shape_79, Training, outputDataPerms, anyDlarrayInputs] = preprocessInput(onnx__Shape_79, params, varargin)
% Parse input arguments
[inputDataPerms, outputDataPerms, Training] = parseInputs(onnx__Shape_79, 1, params, varargin{:});
anyDlarrayInputs = any(cellfun(@(x)isa(x, 'dlarray'), {onnx__Shape_79}));
% Make the input variables into unlabelled dlarrays:
onnx__Shape_79 = makeUnlabeledDlarray(onnx__Shape_79);
% Permute inputs if requested:
onnx__Shape_79 = permuteInputVar(onnx__Shape_79, inputDataPerms{1}, 3);
end
