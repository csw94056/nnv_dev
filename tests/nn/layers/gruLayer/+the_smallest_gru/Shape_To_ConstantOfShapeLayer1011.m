classdef Shape_To_ConstantOfShapeLayer1011 < nnet.layer.Layer & nnet.layer.Formattable
    ...
    methods
        function this = Shape_To_ConstantOfShapeLayer1011(name, onnxParams)
            this.Name = name;
            this.OutputNames = {'hidden'};
            this.ONNXParams = onnxParams;
        end
        
        function [hidden] = predict(this, input)
            if isdlarray(input)
                input = stripdims(input);
            end
            inputNumDims = 3;
            onnxParams = this.ONNXParams;
            [hidden, hiddenNumDims] = Shape_To_ConstantOfShapeFcn(input, inputNumDims, onnxParams, 'Training', false, ...
                'InputDataPermutation', {[3 2 1], ['as-is']}, ...
                'OutputDataPermutation', {[3 2 1], ['as-is']});
            if any(cellfun(@(A)~isnumeric(A), {hidden}))
                fprintf('Runtime error in network. The custom layer ''%s'' output a non-numeric value.\n', 'Shape_To_ConstantOfShapeLayer1011');
                error(message('nnet_cnn_onnx:onnx:BadCustomLayerRuntimeOutput', 'Shape_To_ConstantOfShapeLayer1011'));
            end
            hidden = dlarray(single(hidden), 'CB');
            if ~coder.target('MATLAB')
                hidden = extractdata(hidden);
            end
        end
        
        function [hidden] = forward(this, input)
            if isdlarray(input)
                input = stripdims(input);
            end
            inputNumDims = 3;
            onnxParams = this.ONNXParams;
            [hidden, hiddenNumDims] = Shape_To_ConstantOfShapeFcn(input, inputNumDims, onnxParams, 'Training', true, ...
                'InputDataPermutation', {[3 2 1], ['as-is']}, ...
                'OutputDataPermutation', {[3 2 1], ['as-is']});
            if any(cellfun(@(A)~isnumeric(A), {hidden}))
                fprintf('Runtime error in network. The custom layer ''%s'' output a non-numeric value.\n', 'Shape_To_ConstantOfShapeLayer1011');
...
function [hidden, hiddenNumDims, state] = Shape_To_ConstantOfShapeFcn(input, inputNumDims, params, varargin)
...
%
% [HIDDEN] = Shape_To_ConstantOfShapeFcn(INPUT, PARAMS)
...
%			network output(s) in HIDDEN.
%
% [HIDDEN, STATE] = Shape_To_ConstantOfShapeFcn(INPUT, PARAMS)
...
%				  INPUT:		[batch_size, 1, 2]				Type: FLOAT
...
%				  HIDDEN:		[Unknown, Unknown, Unknown]				Type: FLOAT
...
[hidden, hiddenNumDims, state] = Shape_To_ConstantOfSGraph1006(input, NumDims.input, Vars, NumDims, Training, params.State);
% Postprocess the output data
[hidden] = postprocessOutput(hidden, outputDataPerms, anyDlarrayInputs, Training, varargin{:});
end

function [hidden, hiddenNumDims1010, state] = Shape_To_ConstantOfSGraph1006(input, inputNumDims1009, Vars, NumDims, Training, state)
% Function implementing the graph 'Shape_To_ConstantOfSGraph1006'
...
% Shape:
[Vars.onnx__Gather_5, NumDims.onnx__Gather_5] = onnxShape(Vars.input, NumDims.input);

% Gather:
[Vars.onnx__Unsqueeze_7, NumDims.onnx__Unsqueeze_7] = onnxGather(Vars.onnx__Gather_5, Vars.onnx__Gather_6, 0, NumDims.onnx__Gather_5, NumDims.onnx__Gather_6);

% Unsqueeze:
[shape, NumDims.onnx__Concat_11] = prepareUnsqueezeArgs(Vars.onnx__Unsqueeze_7, Vars.UnsqueezeAxes1007, NumDims.onnx__Unsqueeze_7);
Vars.onnx__Concat_11 = reshape(Vars.onnx__Unsqueeze_7, shape);

% Concat:
[Vars.onnx__ConstantOfShap, NumDims.onnx__ConstantOfShap] = onnxConcat(0, {Vars.onnx__Concat_75, Vars.onnx__Concat_11, Vars.onnx__Concat_76}, [NumDims.onnx__Concat_75, NumDims.onnx__Concat_11, NumDims.onnx__Concat_76]);

% ConstantOfShape:
[Vars.hidden, NumDims.hidden] = onnxConstantOfShape(Vars.ConstantOfShapeValue1008, Vars.onnx__ConstantOfShap);

% Set graph output arguments from Vars and NumDims:
hidden = Vars.hidden;
hiddenNumDims1010 = NumDims.hidden;
...
function [hidden] = postprocessOutput(hidden, outputDataPerms, anyDlarrayInputs, Training, varargin)
% Set output type:
...
hidden = permuteOutputVar(hidden, outputDataPerms{1}, 3);
end
