...
xl = -0.056555221080780;
yl = -0.444801310300827;
xu = -0.036555221080780;
yu = -0.424801310300827;
...
% plotting tangent lines and intersection lines
for i = z_max-1:-1:z_min
    if pzs(i, iuy_) == inf
        % tangent line at x-coordiante
        pzs_dz_dx = pzs(z_max, z_) - (   pzs(z_max, x_) - pzs(i, x_)  )*pzs(z_max, dzx_);
        plot3([pzs(z_max, x_) pzs(i, x_)], [pzs(z_max, y_) pzs(i, y_)], [pzs(z_max, z_) pzs_dz_dx], 'r', 'LineWidth',4);
        fprintf('=======| red tangent line slope (x-axis): \t\t x: %f \t\t y: %f\n', pzs(i, dzx_), pzs(i, dzy_));

        % intersection line between global maximum and a point at
        % x-coordinate
        pzs_dz_dx = pzs(z_max, z_) - (   pzs(z_max, x_) - pzs(i, x_)   )*pzs(i,iux_);
        plot3([pzs(z_max, x_) pzs(i, x_)], [pzs(z_max, y_) pzs(z_max, y_)], [pzs(z_max, z_) pzs_dz_dx], 'r--', 'LineWidth',4);
        fprintf('=======| red line intersection slope: \t\t\t x: %f \t\t y: %f \n', pzs(i, iux_), pzs(i, iuy_));

    elseif pzs(i, iux_) == inf
        % tangent line at y-coordinate
        pzs_dz_dy = pzs(z_max, z_) - (   pzs(z_max, y_) - pzs(i, y_)  )*pzs(z_max, dzy_);  %pzs(z_max, dz_);
        plot3([pzs(z_max, x_) pzs(i, x_)], [pzs(z_max, y_) pzs(i, y_)], [pzs(z_max, z_) pzs_dz_dy], 'b', 'LineWidth',4);
        fprintf('=======| blue tangent line slope (y-axis): \t\t x: %f \t\t y: %f\n', pzs(i, dzx_), pzs(i, dzy_));

        % intersection line between global maximum and a point at
        % y-coordinate
        pzs_dz_dy = pzs(z_max, z_) - (   pzs(z_max, y_) - pzs(i, y_)   )*pzs(i,iuy_);
        plot3([pzs(z_max, x_) pzs(i, x_)], [pzs(z_max, y_) pzs(i, y_)], [pzs(z_max, z_) pzs_dz_dy], 'b--', 'LineWidth',4);
        fprintf('=======| blue line intersection slope: \t\t\t x: %f \t\t y: %f \n', pzs(i, iux_), pzs(i, iuy_));

    elseif (pzs(i, iux_) ~= inf) & (pzs(i, iux_) ~= inf)
        % intersection line between global maximum and global minimum
        plot3([pzs(z_max, x_) pzs(i, x_)], [pzs(z_max, y_) pzs(i, y_)], [pzs(z_max, z_) pzs(i, z_)], 'g--', 'LineWidth',4);
        fprintf('=======| green line intersection slope: \t\t x: %f \t\t y: %f \n', pzs(i, iux_), pzs(i, iuy_));
    end
end
...
if (  abs(u_x_min_slope) == abs(pzs(su_max2min_i, iux_))  ) && (  abs(u_y_min_slope) < abs(pzs(su_max2min_i, iuy_))  )
    ...
    if u_x_min_slope ~= u_x_slope(tan_i_)
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % plot: line from global maxima to px
        plot3([pzs(z_max, x_) px(x_)], [pzs(z_max, y_) px(y_)], [pzs(z_max, z_) px(z_)], 'k', 'LineWidth',4);
        
        % plot: points at px and py
        plot3(px(x_), px(y_), px(z_), 'xk','LineWidth',10);
        plot3(py(x_), py(y_), py(z_), '+k','LineWidth', 10);

        % plot: intersection line from px to pxy
        pxy = po;
        pxy(z_) = px(z_) - pxy_inter_y*(yu-yl)
        plot3([px(x_) pxy(x_)], [px(y_) pxy(y_)], [px(z_) pxy(z_)], 'r', 'LineWidth',4);
        plot3(pxy(x_), pxy(y_), pxy(z_), '*r','LineWidth', 10);

        % plot: tanget line from px to pxy
        pxy_g = pxy;
        pxy_g(z_) = px(z_) - pxy_dxdy(y_)*(yu-yl);
        plot3([px(x_) pxy_g(x_)], [px(y_) pxy_g(y_)], [px(z_) pxy_g(z_)], 'm', 'LineWidth',4);
        ...
        if new_z < po(z_)
            ...
            % plot: tangent line from py to pyx (y-axis)
            pyx = po;
            pyx(z_) = py(z_) - x_slope*(xu-xl);

            pyx_g = pyx;
            pyx_g(z_) = py(z_) - pyx_dxdy(y_)*(xu-xl);

            plot3([pzs(z_max, x_) py(x_)], [pzs(z_max, y_) py(y_)], [pzs(z_max, z_) py(z_)], 'k', 'LineWidth',4);
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        end
    end
...
elseif (  abs(u_x_min_slope) < abs(pzs(su_max2min_i, iux_))  ) && (  abs(u_y_min_slope) == abs(pzs(su_max2min_i, iuy_))  )
    ...
    if u_y_min_slope ~= u_y_slope(tan_i_)
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % plot: line from global maxima to py
        plot3([pzs(z_max, x_) py(x_)], [pzs(z_max, y_) py(y_)], [pzs(z_max, z_) py(z_)], 'k', 'LineWidth',4);
        ...
        % plot: intersection line from py to pyx
        pyx = po;
        pyx(z_) = py(z_) - pyx_inter_x*(xu-xl);
        plot3([py(x_) pyx(x_)], [py(y_) pyx(y_)], [py(z_) pyx(z_)], 'r', 'LineWidth',4);
        plot3(pyx(x_), pyx(y_), pyx(z_), '*r','LineWidth', 10);

        % plot: tanget line from py to pyx
        pyx_g = pyx;
        pyx_g(z_) = py(z_) - pyx_dxdy(y_)*(xu-xl);
        plot3([py(x_) pyx_g(x_)], [py(y_) pyx_g(y_)], [py(z_) pyx_g(z_)], 'm', 'LineWidth',4);
        ...
        if new_z < po(z_)
            ...
            % tangent line from px to pxy (x-axis)
            pxy = po;
            pxy(z_) = px(z_) - y_slope*(yu-yl);
            plot3([px(x_) pxy(x_)], [px(y_) pxy(y_)], [px(z_) pxy(z_)], 'm--', 'LineWidth',4);
...
elseif (u_x_slope_i ~= tan_i_) && (u_x_slope_i ~= int_max2min_i_)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % plot: a line based on u_y_min_slope from px to pxy
    px = pzs(px_i, x_:z_);
    % a point opposite to global maximum on x-axis and y-axis
    po = pzs(su_max2min_i, x_:z_);
    
    pxy = po;
    pxy(z_) = px(z_) - u_y_min_slope*(yu-yl);
    plot3([px(x_) pxy(x_)], [px(y_) pxy(y_)], [px(z_) pxy(z_)], 'k', 'LineWidth',4);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
elseif (u_y_slope_i ~= tan_i_) && (u_y_slope_i ~= int_max2min_i_)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % plot: a line based on u_x_min_slope from py to pyx
    py = pzs(py_i, x_:z_);
    % a point opposite to global maximum on x-axis and y-axis
    po = pzs(su_max2min_i, x_:z_);
    
    pyx = po;
    pyx(z_) = pyx(z_) - u_x_min_slope*(xu-xl);
    plot3([pzs(z_max, x_) pyx(x_)], [pzs(z_max, y_) pyx(y_)], [pzs(z_max, z_) pyx(z_)], 'k', 'LineWidth',4);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
end
