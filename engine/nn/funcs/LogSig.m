    ...
    methods(Static)
        function S = reach_sparse_star_approx(varargin)
            ...
            if strcmp(method, 'approx-sparse-star-no-split') || strcmp(method, 'approx-sparse-star')
                if relaxFactor == 0
                    S = LogSig.sparse_multiStepLogSig_NoSplit(I, dis_opt, lp_solver);
                else
                    S = LogSig.sparse_relaxedMultiStepLogSig_NoSplit(I, relaxFactor, dis_opt, lp_solver);
                end
            else
                error('Unknown reachability method');
            end
...
methods(Static) % main reach method
    ...
    function R = reach(varargin)
        ...
        elseif strcmp(method, 'approx-sparse-star')
            ...
            R = LogSig.reach_sparse_star_approx(I, method, relaxFactor, dis_opt, lp_solver);
